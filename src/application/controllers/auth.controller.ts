...
export default class AuthController {
  ...
  signupUser = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const userPhoneNumberExists = await this.userRepository.isExists(undefined, phoneNumber);
      ...
      const otpResponse: SendOtpResponse = await this.otpService.sendVerificationMessage(phoneNumber, otpCode);
  ...
  loginUser = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const user = await this.userRepository.findOne({ phoneNumber }, selectPassword);
  ...
  loginManager = async (req: Request, res: Response) => {
    ...
    try {
      const selectPassword = true;
      const manager = await this.managerRepository.findOne({ username }, selectPassword);
  ...
  forgotPassword = async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(200).json({
        success: true,
        message: ResponseMessages.RES_MSG_USER_FORGET_PASSWORD_CODE_SENT_SUCCESSFULLY_EN,
        userId: user.id
      });
    } catch (err) {
      const errorMessage = getErrorMessage(err);
