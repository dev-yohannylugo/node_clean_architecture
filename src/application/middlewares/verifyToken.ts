...
const verifyToken = async (req: Request, res: Response, next: NextFunction) => {
  // check manager token
  const managerToken: string | undefined = req.header('admin-access-token')?.replace('Bearer ', '');
  ...
  // check user token
  const userToken: string | undefined = req.header('x-access-token')?.replace('Bearer ', '');
