...
const managerRepository: IManagerRepo = container.get<IManagerRepo>(Types.IManagerRepo);
...
const directorAuth = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
const MonitorAuth = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (manager.role !== Role.MANAGER && manager.role !== Role.DIRECTOR && manager.role !== Role.MONITOR) {
      return res.status(403).json({
        success: false,
        message: ResponseMessages.RES_MSG_UNAUTHORIZED_CLIENT_EN
      });
