...
import { CreateParams, UserEntity, Query } from '../../../domain/entities/user.entity';
...
export default class UserRepoImpl implements IUserRepo {
  async findPaging(query: Query, offset: number, limit: number, sort?: {}): Promise<UserEntity[]> {
    const queryObj = { ...Mapper.toQuery(query) };
    const sortObj = sort || { _id: -1 };

    const docs = await User.find(queryObj).skip(offset).limit(limit).sort(sortObj);
  ...
  async isExists(email?: string, phoneNumber?: string, id?: string): Promise<boolean> {
  ...
  async findOne(query: Query, select: boolean): Promise<UserEntity | null> {
    ...
    if (select) doc = await User.findOne({ ...Mapper.toQuery(query) }).select('+password');
